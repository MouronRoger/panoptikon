...
def test_invalid_handler_types(event_bus: EventBus) -> None:
    """Test handling of invalid handler types."""
    # Try to use an invalid handler type
    invalid_handler = "not a callable or handler"  # type: ignore
...
def test_event_source_auto_detection(event_bus: EventBus) -> None:
    ...
    assert "test_events_extended.py" in event.source
...
def test_no_event_loop_error() -> None:
    """Test error when no event loop is available for async delivery."""
    event_bus = EventBus()
...
def test_synchronous_invalid_handler(event_bus: EventBus) -> None:
    """Test synchronous delivery to an invalid handler type."""

    # Create a non-callable object rather than a MagicMock
    class NonCallableObject:
        """A simple non-callable object."""

        pass

    invalid_handler = NonCallableObject()

    # Create a fake subscription with this handler
    subscription = MagicMock()
    subscription.handler = invalid_handler
    subscription.delivery_mode = EventDeliveryMode.SYNCHRONOUS

    # Check that event_bus logs the error instead of raising it
    with patch("src.panoptikon.core.events.logger.error") as mock_logger:
        event_bus._deliver_event(TestEvent(), subscription)
        mock_logger.assert_called_once()
        # Check that the error message contains the expected text
        args, _ = mock_logger.call_args
        assert "Invalid handler type" in args[0]


def test_asynchronous_invalid_handler(event_bus: EventBus) -> None:
    """Test asynchronous delivery to an invalid handler type."""
